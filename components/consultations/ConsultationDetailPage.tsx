...
export default function ConsultationDetailPage() {
  ...
  useEffect(() => {
    const fetchConsultation = async () => {
      try {
        const response = await fetch(`/api/consultation/list?warehouseId=${warehouseId}`)
        const consultations = await response.json()
        const consultationData = consultations.find((c: any) => c.invoiceNo === params.consultationId)
